...
async function onSubmit(event) {
  ...
  if (actionValue === "create") {
    ...
    const resourceDescription = $("resourceDescription")?.value ?? "";
    
    if (resourceName.trim().length < 5 || resourceName.trim().length > 30 ||
        resourceDescription.trim().length < 10 || resourceDescription.trim().length > 50) {
