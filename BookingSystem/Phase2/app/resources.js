...
function isResourceDescriptionValid(value) {
  const trimmed = value.trim();
  // Match the hint in HTML: 10-50 characters
...
function attachResourceDescriptionValidation(textarea) {
  const update = () => {
    ...
    const nameValid = nameInput ? isResourceNameValid(nameInput.value) : false;
    
    if (raw.trim() === "") {
      setInputVisualState(textarea, "neutral");
      setButtonEnabled(createButton, nameValid);
      return;
    }
    ...
    // Enable Create button only if BOTH fields are valid
    setButtonEnabled(createButton, nameValid && valid);
  };
...
// Attach validation to description if it exists
if (resourceDescription) {
  attachResourceDescriptionValidation(resourceDescription);
}
