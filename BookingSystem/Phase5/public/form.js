...
function buildValidationMessage(errors) {
  ...
  const lines = errors.map((e) => {
    ...
    if (field === 'resourceName') friendlyField = 'Resource Name';
    if (field === 'resourcePrice') friendlyField = 'Price';
    if (field === 'resourcePriceUnit') friendlyField = 'Price Unit';
...
async function onSubmit(event) {
  ...
  // Get resource name for use in messages
  const resourceName = $("resourceName")?.value ?? "";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        // CHANGED: More user-friendly duplicate message
        const resourceName = payload.resourceName;
        const msg = `⚠️ Cannot add "${resourceName}" - a resource with this name already exists.\n\nPlease choose a different name or check the Resources list to see existing resources.`;
    ...
    // -----------------------------------------
    // CHANGED: More user-friendly success message
    // We expect: { ok: true, data: {...} }
    const resourceName = body?.data?.name || payload.resourceName;
    const resourcePrice = body?.data?.price || payload.resourcePrice;
    const resourceUnit = body?.data?.price_unit || payload.resourcePriceUnit;
    
    const msg = `✅ Success! "${resourceName}" (${resourcePrice} €/${resourceUnit}) has been added to the system.\n\nYou can now use this resource for bookings.`;
    ...
    showFormMessage("error", "❌ Network error: Could not reach the server. Please check:\n• Your internet connection\n• That the server is running\n• And try again.");
  }
}