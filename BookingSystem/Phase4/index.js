...
const app = express();
const PORT = process.env.PORT || 5000;  // FIXED: Bug 1
...
const resourceValidators = [
  ...
  body('resourceDescription')
    ...
    .matches(/^[A-Za-z0-9 .,!?-]+$/).withMessage('Description contains invalid characters'),
  ...
  body('resourcePriceUnit')
    ...
    .trim()
    .isIn(['hour', 'day', 'week', 'month'])  // FIXED: Bug 2
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  // FIXED: Bug 3 - removed the line that sets resourceAvailable = false
  ...
  try {
    ...
    `;
    const params = [
      resourceName,  // FIXED: Bug 4 - store actual name, not hash
      resourceDescription,
      Boolean(resourceAvailable),
      Number(resourcePrice),  // FIXED: Bug 5 - don't multiply by 2
      resourcePriceUnit
    ];
...
// GET endpoint - Get all resources
...
app.listen(PORT, () => {
  ...
  console.log('✅ Validation enabled with express-validator');
  console.log('✅ PostgreSQL database integration enabled');
});